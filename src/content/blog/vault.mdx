...
date: 2024-06-28
title: 'Vault'
image: '/images/blog-images/covers/vault.jpg'
description: 'Securely store and recover keys'
author: andreas
tags: ['launchweek', 'marketing']
---

_The most requested feature for Viraza has finally arrived._
...
Both the [getKey](/docs/api-reference/v2/keys/get-api-key) and [listKeys](/docs/api-reference/v2/apis/list-api-keys) endpoints accept a `decrypt` query parameter. If you set this to `true`, the key will be decrypted and returned in the response as `plaintext`.

When recovering keys, your root key must have permission to decrypt. Head over to the [dashboard](https://viraza.com/settings/root-keys) and make sure the `decrypt_key` permission is enabled.

```shell
curl --request GET \
  --url https://api.viraza.net/v1/keys.getKey?keyId={KEY_ID}&decrypt=true \
...
2. **Contact us:**<br/>
   To opt in to recovery, send us an email at info@viraza.net. Send us the email from the email address associated with your workspace and include the API ID that you want to enable recovery for.

## Documentation

You can find the documentation [here](https://viraza.net/docs/security/recovering-keys).

## Support

If you have any questions about recovery, please reach out to us on [Discord](https://viraza.net/discord) or at [info@viraza.net](mailto:info@viraza.net).

For security concerns, please disclose them responsibly by emailing [security@unkey.com](mailto:security@unkey.com) instead.
