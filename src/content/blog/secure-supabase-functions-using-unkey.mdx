---
date: 2023-10-03
title: 'Secure your Supabase functions with Viraza'
image: '/images/blog-images/covers/unkey-supabase.jpg'
description: 'Learn how to use Viraza to secure your Supabase functions'
author: james
tags: ['tutorials']
---

Supabase offers [edge functions](https://supabase.com/docs/guides/functions) built upon Deno. They have a variety of uses for applications like OpenAI or working with their storage product. In this post, we will show you how to use Viraza to secure your function in just a few lines of code.

## What is Viraza?

Viraza is an open source API management platform that helps developers secure, manage, and scale their APIs. Viraza has built-in features that can make it easier than ever to provide an API to your end users, including:
...
2. Create a [Viraza account](https://viraza.net) and follow our [Quickstart guide](https://viraza.net/docs/quickstart). So you have an API key to verify.
...
Now that Supabase is setup, we can create a Supabase function. This function will be where we secure it using Viraza.
...
## Add Viraza to secure our Supabase function

### Add `verifyKey` to our function

Now that we have a function, we must add Viraza to secure the endpoint. Supabase uses Deno, so instead of installing our npm package, we will use ESM CDN to provide the `verifyKey` function we need.
...
Viraza's `verifykey` lets you verify a key from your end users. We will return a result and you can decide whether to give the user access to a resource or not based upon that result. For example, a response could be:
...
First, let's remove the boilerplate code from the function so we can work on adding Viraza.
...
In this post, we have covered how to use Viraza with Supabase functions to secure them. You can check out the code for this project in our [Examples folder](https://github.com/unkeyed/examples/tree/main/supabase-functions)
