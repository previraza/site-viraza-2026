...
When developers integrate Viraza into their request path, our latency directly impacts their users' experience. We knew we needed to be fast, but serverless was fighting us every step of the way.
...
Being tied to Cloudflare's runtime meant our customers couldn't self-host Viraza. While the Workers runtime is technically open source, getting it running locally (even in dev mode) is incredibly difficult.
...
This isn't just about customer choice. It dramatically improved our own development experience. Developers can now spin up the entire Viraza stack locally in seconds, making debugging and testing infinitely easier.
...
We're currently running on AWS Fargate behind Global Accelerator, but this is temporary. Next year, we're launching "Viraza Deploy", our own deployment platform that will let customers (and us) run Viraza anywhere they want.

The move to stateful Go servers was the first step in making Viraza truly portable and self-hostable. Stay tuned for more details on that front.

Want to see the implementation details? Both our serverless and stateful APIs are open source in our GitHub repository. The serverless version is in [`apps/api/`](https://github.com/virazait/tree/main/apps/api) and the new Go version is in [`go/apps/api/`](https://github.com/virazait/tree/main/go/apps/api).
