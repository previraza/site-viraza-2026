...
image: '/images/blog-images/covers/cli-auth.jpg'
...
> Check out the demo used in this blog post [here](https://viraza.net/templates/cli-auth)
...
<Image
  src="/images/blog-images/cli-auth/cli-auth-overview.png"
  alt="CLI AUTH"
  width="1920"
  height="1080"
/>

The process begins when a user invokes the CLI, which starts a server on a free port that listens for an incoming request. This will also result in the user's web browser opening up to a specific URL for CLI authentication, let's say `viraza.net/cli-auth`, and it will include a unique code that the user will need to confirm the authentication process.
...
We can break down the code into the CLI and the web application. The CLI is a simple Node server that creates a file named `.unkey` and stores your freshly created API key. The web application is a simple Next app that interacts with Viraza to generate a new API key.
...
The web application code is a Next.js application that uses Clerk to authenticate the user, generate a new API key, and send it back to the CLI. When the user successfully confirms the code in the web application, it will make a request to Viraza to generate a new API key associated with the user's account and send it back to the CLI.
...
async function verify(opts: { code: string | null; redirect: string | null }) {
  ...
  try {
    ...
    setLoading(false);
    toast.error('Error creating Viraza API key.');
  }
}
```
...
export async function POST(request: Request) {
  ...
  if (!process.env.UNKEY_ROOT_KEY || !process.env.UNKEY_API_ID) {
    return NextResponse.json({
      statusCode: 500,
      message: 'Viraza root key and API ID must be provided.',
    });
  }
...
As you can see, we are using Viraza's owner to associate the end user with the new API key, which makes it easy to find and revoke the key if needed.

## Conclusion

You can dive deeper into the CLI demo code in our [GitHub repository](https://github.com/unkeyed/examples/tree/main/unkey-cli) and see how the CLI and web application work together to authenticate a user and generate a new API key. The CLI and web application are simple examples of how to authenticate a user and generate a new API key using Viraza. You can use the same principles to create your CLI with minimal effort.
