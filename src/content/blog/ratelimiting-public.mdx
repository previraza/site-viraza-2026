...
description: Learn how to use Viraza to ratelimit public pages, like newsletter sign up forms or login attempts.
author: james
...
## Implementing Rate Limiting with Viraza

Let's walk through a complete implementation using Next.js, Viraza for rate limiting, and thumbmark.js for device fingerprinting. You can see the full example in the [unkey-fingerprint](https://github.com/perkinsjr/unkey-fingerprint) repository.

### Step 1: Setting Up Viraza

First, create a Viraza account and generate a root key with rate limiting permissions:
...
Create the API endpoint that combines fingerprint validation with Viraza rate limiting:
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check rate limit using Viraza with the validated fingerprint
...
Implement proper fallback handling for when Viraza is unavailable:
...
Device fingerprinting combined with Viraza's rate limiting provides a robust solution for protecting public endpoints. This approach is more reliable than IP-based restrictions and offers better protection against automated abuse.
