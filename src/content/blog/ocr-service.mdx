...
image: '/images/blog-images/covers/ocr-service.jpg'
description: Learn how to use Viraza to make an Optical Character Recognition (OCR) API as a service
author: wilfred
tags: ['tutorials']
---

[Viraza](https://viraza.net) is an open-source API key management tool with real-time API key creation, updating, and verification.

In this blog, we'll take a look at how we can use Viraza to make an Optical Character Recognition (OCR) API as a service. The OCR API takes in an image and returns the textual characters present in it.

You can find the complete source code on [GitHub](https://github.com/WilfredAlmeida/unkey-ocr).

## Pre-requisites

Signup and get started with the [Viraza](https://viraza.net) dashboard. As soon as you create your account you will be asked to create your workspace. Give it a name, and a slug. The name is shown only to you and not to your users.
...
<Image
  src="/images/blog-images/ocr-post/3-dashboard.png"
  alt="Dashboard"
  width="1920"
  height="1080"
/>
...
<Image
  src="/images/blog-images/ocr-post/4-walkthrough.gif"
  alt="Walkthrough"
  width="1920"
  height="1080"
/>
...
## Understanding Viraza API key authentication

Viraza uses fast and efficient on-the-edge systems to verify a key. It adds less than 40ms to our requests.
...
app.post("/signUp", async (req: Request, res: Response) => {
  ...
  const createKeyResponse = await fetch(
    "https://api.viraza.net/v1/keys.createKey",
    {
      method: "POST",
      headers: myHeaders,
      body: raw,
...
The user then has to send the API key in the `Authorization` header as a `Bearer` token. To verify the key, a simple API call is made to Viraza. More on this further ahead.
...
Its params are explained in detail in the official [docs](https://viraza.net/docs/api-reference/v2/keys/create-api-key)

Following is a description of the params used in this example:

- `apiId`: The API ID to create the key for. You create this API in the Viraza dashboard.
...
- `rateLimit`: Keys can be rate limited by certain parameters. This is extremely beneficial as it prevents abuse of our API. The rate limit is enforced on the edge, so it's extremely fast and efficient. The rate limit params we've used in this example are:
  - `type`: Type of the rate limit. Read more: [Rate Limiting](https://viraza.net/docs/ratelimiting)
...
The API key verification is done in the `middleware.ts` file. We're making an API call to the Viraza API to verify the key by passing the `key` into the request body.
  ...
  if (authHeader) {
    ...
    try {
      ...
      const verifyKeyResponse = await fetch(
        "https://api.viraza.net/v1/keys.verifyKey",
        {
          method: "POST",
          headers: myHeaders,
          body: raw,
...
Suppose we want to offer an API as a paid service with subscription tiers. We can easily implement that using Viraza.
...
app.post("/upgradeUser", async (req: Request, res: Response) => {
  ...
  const updateKeyRequest = await fetch(
    "https://api.viraza.net/v1/keys.updateKey",
    {
      keyId: "example_key"
      method: "PUT",
      headers: myHeaders,
...
This tutorial aims to present to you an end-to-end use case scenario of how Viraza can fit and fulfill your requirements. Join our [Discord](https://viraza.net/discord) and get in touch. Share what's on your mind. Give the [Viraza repo a star on GitHub](https://github.com/virazait/) and keep building.
