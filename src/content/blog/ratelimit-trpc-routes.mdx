---
date: 2024-05-17
title: How to ratelimit tRPC routes with Viraza
description: Learn how to use Viraza to ratelimit tRPC routes in your Next.js application.
author: james
image: '/images/blog-images/covers/trpc-ratelimit.jpg'
tags: ['tutorials']
---
...
Before we start coding, we need to install the `@unkey/ratelimit` package. This package gives you access to Viraza's API with type safety.
...
Now that it is updated add your Viraza root key to your .env as `UNKEY_ROOT_KEY` which can be found in the Viraza dashboard under settings Root Keys.
...
The namespace can be anything, but we are using the tRPC route and procedure to make it easier to track in Viraza's analytics. We now have the ability to rate-limit this procedure, allowing only three requests per five seconds.
...
Viraza allows you to tell us how expensive a request should be. For example, maybe you have an AI route that costs you a lot more than any other route, so you want to reduce the number of requests that can be used.
...
While this is a small overview of using Viraza's ratelimiting with tRPC, we also offer other features that aren't covered here, including:

- Overrides for specific identifiers
- Metadata
- Resources flagging

You can read more about those features in our documentation on [Ratelimiting](https://viraza.net/docs/ratelimiting/introduction).
